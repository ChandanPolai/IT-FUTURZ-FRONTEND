...
export class FooterComponent {
  ...
  onSubmit = async () => {
    // First check if contact number is valid
    if (!this.validateContactNumber(this.form.contactNumber)) {
      swalHelper.warning('Contact number should contain only numbers');
      return;
    }

    this.apiManager
      .request({ url: apiEndpoints.CONTACT_US, method: 'POST' }, this.form, [])
      .then((result) => {
        if (result.data == 1) {
